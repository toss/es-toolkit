    ...
    <table>
      ...
      <tbody>
        <tr v-for="row in rows" :key="row.name">
          ...
          <td v-for="(val, i) in row.values" :key="i">{{ formatTimes(val) }}</td>
          <td v-if="seriesKeys.length >= 2">{{ formatDifference(row.diffPercent) }}</td>
        </tr>
      </tbody>
    </table>
  </div>
