...
::: warning `es-toolkit`의 [`chunk`](../../../reference/array/chunk.md)를 사용하세요

이 `chunk` 함수는 `null`이나 `undefined` 처리, `size` 기본값 처리 등으로 인해 느리게 동작해요.

대신 더 빠르고 현대적인 `es-toolkit`의 [chunk](../../../reference/array/chunk.md)를 사용하세요.
...
- `size?` (`number`): 각 작은 배열의 크기예요. 기본값은 `1`이에요.
