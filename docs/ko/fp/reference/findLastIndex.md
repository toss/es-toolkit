# findLastIndex (함수형 프로그래밍)

조건을 만족하는 마지막 값의 인덱스를 반환하는 함수를 만들어요. 함수형 프로그래밍의 [`pipe`](./pipe.md) 와 같이 사용해요.
...
(`(array: readonly T[]) => number`): `readonly T[]`를 마지막 일치 인덱스나 `-1`로 변환하는 함수예요.
