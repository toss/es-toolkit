...
- `shouldPick` (`(value: T[keyof T], key: ObjectKeys<T>) => boolean`): 속성을 선택할지 결정하는 조건 함수예요. 값과 키를 받아서 선택하려면 `true`, 제외하려면 `false`를 반환해요. 숫자 키는 조건 함수에 문자열로 전달돼요 ([`ObjectKeys<T>`](/ko/types/reference/objects/ObjectKeys) 참고).
