...
- `options?` (`DebounceOptions`): 옵션 객체.
  - `signal?` (`AbortSignal`): 디바운스된 함수를 취소하기 위한 선택적 `AbortSignal`.
  - `edges?` (`Array<'leading' | 'trailing'>`): 원래 함수를 언제 실행할지 나타내는 배열. 기본값은 `['trailing']`이에요.
