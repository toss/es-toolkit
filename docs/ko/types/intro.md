...
| [`ToCamelCaseKeys<T>`](./reference/ToCamelCaseKeys.md)       | 모든 키를 재귀적으로 camelCase로 변환해요. `toCamelCaseKeys`의 반환 타입이에요.        |
| [`ToSnakeCaseKeys<T>`](./reference/ToSnakeCaseKeys.md)       | 모든 키를 재귀적으로 snake_case로 변환해요. `toSnakeCaseKeys`의 반환 타입이에요.       |
| [`ToPascalCaseKeys<T>`](./reference/ToPascalCaseKeys.md)     | 모든 키를 재귀적으로 PascalCase로 변환해요. `toPascalCaseKeys`의 반환 타입이에요.      |
| [`ToKebabCaseKeys<T>`](./reference/ToKebabCaseKeys.md)       | 모든 키를 재귀적으로 kebab-case로 변환해요. `toKebabCaseKeys`의 반환 타입이에요.       |
| [`ToConstantCaseKeys<T>`](./reference/ToConstantCaseKeys.md) | 모든 키를 재귀적으로 CONSTANT_CASE로 변환해요. `toConstantCaseKeys`의 반환 타입이에요. |
