...
Instead, use the faster and more modern [`dropWhile`](../../../reference/array/dropWhile.md) from `es-toolkit`.
...
- `predicate?` (`ListIteratee<T>`): The predicate function to apply to each element. Can accept a function, object pattern, array pattern, or property name. Default is `identity`.
