...
| [`Merge<T, S>`](./reference/objects/Merge.md)                        | 把一个对象类型深度合并进另一个对象类型的结果。`merge` 的返回类型。 |
| [`ObjectKeys<T>`](./reference/objects/ObjectKeys.md)                 | 以 `Object.keys` 返回的形式创建键的联合。`ValueOf` 的键侧对应物。  |
