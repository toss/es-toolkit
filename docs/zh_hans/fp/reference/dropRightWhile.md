# dropRightWhile (函数式编程)

创建一个在谓词通过时丢弃末尾值的函数。与函数式编程的 [`pipe`](./pipe.md) 一起使用。
...
- `predicate` (`(item: T) => boolean`): 判断是否应移除末尾值的函数。

#### 返回值

(`(array: readonly T[]) => T[]`): 一个将 `readonly T[]` 映射为从末尾移除值后剩余数组的函数。
