...
请改用更快、更现代的 `es-toolkit` 的 [`omitBy`](../../../reference/object/omitBy.md)。
...
- `predicate?` (`ValueKeyIteratee<T[keyof T]> | ValueKeyIteratee<T>`): 对每个属性执行的断言函数。此函数返回true的属性将被删除。默认为 `identity` 函数。
