...
当您想要通过异步转换每个值来创建新对象时,请使用 `mapValuesAsync`。键保持不变,只有值会更改为 `getNewValue` 函数返回的 Promise 中的值。
...
- `options` (`MapValuesAsyncOptions`, 可选): 控制并发数的选项。
  - `concurrency` (`number`, 可选): 最大并发操作数。如果未指定,所有操作将并发运行。

#### 返回值

(`Promise<Record<K, V>>`): 返回一个包含值被转换后的新对象的 Promise。
