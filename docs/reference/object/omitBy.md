...
- `shouldOmit` (`(value: T[keyof T], key: ObjectKeys<T>) => boolean`): A condition function that determines whether to exclude a property. Receives the value and key, and returns `true` to exclude, `false` to keep. Numeric keys are passed to the condition function as strings (see [`ObjectKeys<T>`](/types/reference/objects/ObjectKeys)).
