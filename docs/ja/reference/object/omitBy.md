...
条件関数を満たさないプロパティで構成された新しいオブジェクトを返します。`obj`にインデックスシグネチャがあるかどうかによって、戻り値の型が変わります。

- `obj`に**文字列または数値のインデックスシグネチャ**がある場合（例: `Record<string, number>`）: `T` を返します — このような型はもともとどのキーも存在しない可能性があるため、結果を元のオブジェクトが必要な場所にそのまま渡せます。
- それ以外の場合（例: `{ a: number; b: string }`）: `Partial<T>` を返します — 既知のプロパティのいずれかが除外されている可能性があるためです。
