...
| [`Merge<T, S>`](./reference/Merge.md)                        | あるオブジェクト型に別のオブジェクト型を深くマージした結果です。`merge` の戻り値の型です。 |
| [`ObjectKeys<T>`](./reference/ObjectKeys.md)                 | `Object.keys` が返す形でキーのユニオンを作ります。`ValueOf` のキー版です。                 |
