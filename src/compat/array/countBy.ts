...
export function countBy(collection: any, iteratee?: any): Record<string, number> {
  ...
  const array = isArrayLike(collection) ? Array.from(collection) : Object.keys(collection).map(key => collection[key]);
  const mapper = iterateeToolkit(iteratee == null ? undefined : iteratee) as (value: any) => any;
  ...
  for (let i = 0; i < array.length; i++) {
    ...
    result[key] = (result[key] == null ? 0 : result[key]) + 1;
  }
