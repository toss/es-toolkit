...
/**
 * Creates a data-last operator that returns the first `count` elements of an
 * array. If `count` is greater than the array length, the whole array is
 * returned.
 *
 * The returned operator is **lazy-capable** for a non-negative integer `count`:
 * inside a {@link pipe} it stops the walk as soon as `count` elements have been
 * collected, so preceding lazy operators never process the rest of the input.
 *
...
export function take<T>(count: number): (array: readonly T[]) => T[] {
  const dataLast = (array: readonly T[]): T[] => takeToolkit(array, count);
