import { partition as partitionToolkit } from '../../array/partition';
 ...
 * whether an element should be placed in the truthy array. The function is called with each
 * element of the array.
 * @returns {(arr: T[]) => [T[], T[]]} A function that receive the array to partition as argument and returns a tuple containing two arrays: the first array contains elements for.
 * which the predicate returned true, and the second array contains elements for which the
 * predicate returned false.
 *
 * @example
 ...
 * // even will be [2, 4], and odd will be [1, 3, 5]
 */
export function partition<T>(isInTruthy: (value: T) => boolean): (arr: readonly T[]) => [truthy: T[], falsy: T[]];

export function partition<T>(
  arrOrIsInTruthy: readonly T[] | ((value: T) => boolean),
  isInTruthy?: (value: T) => boolean
) {
  if (isInTruthy == null) {
    return (arr: readonly T[]) => partition(arr, arrOrIsInTruthy as (value: T) => boolean);
  }

  const arr = arrOrIsInTruthy as readonly T[];
  return partitionToolkit(arr, isInTruthy);
}
