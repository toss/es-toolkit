...
describe('mapKeys', () => {
  ...
  it('(curried) should iterate over and map the object using its own string keys', () => {
    expect(mapKeys<Record<string, number>>((_, key) => `${key}a`)({ a: 1, b: 2, c: 3 })).toEqual({
      aa: 1,
      ba: 2,
      ca: 3,
    });
  ...
  it('(curried) should iterate over and map the object using its own number keys', () => {
    expect(mapKeys<Record<number, string>>((_, key) => key * 2)({ 1: 'a', 2: 'b', 3: 'c' })).toEqual({
      2: 'a',
      4: 'b',
      6: 'c',
    });
  ...
  it('(curried) should pass the value corresponding to the current key into the iteratee', () => {
    expect(mapKeys<Record<string, number>>(value => value)({ a: 1, b: 2, c: 3 })).toEqual({ 1: 1, 2: 2, 3: 3 });
  });
});
