import { toMerged as toMergedToolkit } from '../../object/toMerged';
 ...
 *
 * @param {S} source - The source object whose properties will be merged into the cloned target object.
 * @returns {(target: T) => T & S} A function that receive the target object to be cloned and merged into. This object is not modified directly as argument and returns a new object with properties from the source object merged into a deep clone of the target object.
 *
 * @template T - Type of the target object.
 * @template S - Type of the source object.
 *
...
export function toMerged<T extends Record<PropertyKey, any>, S extends Record<PropertyKey, any>>(
  ...
  if (source == null) {
    return (target: T) => toMerged(target, targetOrSource as S);
  }

  const target = targetOrSource as T;
  return toMergedToolkit(target, source);
}
