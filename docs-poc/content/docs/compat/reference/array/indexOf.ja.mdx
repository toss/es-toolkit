...
<Callout type="warn" title="`Array.prototype.indexOf` または `Array.prototype.findIndex` を使用してください">
...
<Callout type="info" title="`array` は `ArrayLike<T>` または `null` または `undefined` である可能性があります">
