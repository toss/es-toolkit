...
<Callout type="warn" title="`Array.prototype.indexOf`나 `Array.prototype.findIndex`를 사용하세요">
...
<Callout type="info" title="`array`는 `ArrayLike<T>`이거나 `null` 또는 `undefined`일 수 있어요">
