import { DocNodeClass, DocNodeFunction } from "@deno/doc";
...
export function formatClassDoc(
  node: DocNodeClass,
  ...
  if (node.classDef.decorators != null && node.classDef.decorators.length > 0) {
    result += node.classDef.decorators.map((d) => formatDecorator(d, options))
      .join("\n");
  }
  ...
  if (node.classDef.typeParams.length > 0) {
    result += "<";
    result += node.classDef.typeParams.map((p) => formatTypeParam(p, options))
      .join(", ");
    result += ">";
  }
  ...
  if (node.classDef.superTypeParams.length > 0) {
    result += "<";
    result += node.classDef.superTypeParams.map((p) => formatType(p, options))
      .join(", ");
    result += ">";
  }
