...
- **`{简短示例代码}`**：使用描述性变量名（而不是具体的值）的最小示例，使读者能从中推断出函数接口。
  > 示例：`const chunked = chunk(arr, size);`
- **`{参考 n}`**：为每个重载提供简短的函数签名（例如 `chunk(arr, size)`）。尽量合并为一个；只在行为确实不同时（例如对数组和对象有不同行为）才拆分为多个。
- **`{参考 n 的详细说明}`**：包括何时使用该函数会有用，并通过短小的内联示例帮助读者理解。使用自然流畅的句子，避免使用"说明：…"这种冒号式风格。
- **`{参考 n 的参数}`**：列出每个参数的名称、类型和说明。可选参数请标注"可选"并写出默认值。
  > 示例：`` - `arr` (`ArrayLike<T>`): 要拆分的数组。 ``
  > 示例：`` - `size` (`number`, 可选): 每个较小数组的大小。必须是大于 1 的整数。默认值为 `1`。 ``
...
::: warning 请使用 `es-toolkit` 的 `chunk`

这个 `chunk` 函数因为要处理 `null`、`undefined` 以及 `size` 的默认值，所以运行较慢。

请改用更快、更现代的 `es-toolkit` 的 [chunk](../../array/chunk.md)。
...
当你想把一个长数组拆分成多个长度相同的较小数组时，请使用 `chunk`。如果数组无法被均匀拆分，最后一个数组会包含剩余的元素。
...
- `arr` (`ArrayLike<T>`): 要拆分的数组。
- `size` (`number`, 可选): 每个较小数组的大小。必须是大于 1 的整数。默认值为 `1`。
