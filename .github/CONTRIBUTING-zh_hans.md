...
我们欢迎针对 Lodash 自身类型所允许的输入的修复。`es-toolkit/compat` 与 Lodash 行为完全一致非常重要——一旦返回值不同，正在迁移的项目就会以很难排查的方式出问题。

不过，Lodash 也支持一些遗留行为，例如在字符串位置传入 `null` 或数字这类无效输入。每一项这样的行为都会给频繁执行的代码增加分支，而对应的调用在有类型的项目里根本写不出来。因此，只有满足以下至少一条时，我们才接受修复这类差异：

1. **该调用能在 TypeScript `strict` 模式下通过 `@types/lodash` 的编译。** 请以 `import { fn } from 'lodash'` 的形式编写复现代码，并用 `tsc` 检查。如果类型拒绝了这个调用，这个差异就不在范围内。
2. **真实代码里存在这样的调用。** 请链接到 GitHub 公开仓库中传入这类输入的文件 permalink。它必须是其他人在使用的项目：不能是你为了展示 bug 而创建的仓库，不能是 fork，也不能是 Lodash 自己的测试代码——测试只说明 Lodash 如何运行，并不说明有人如何使用它。

如果两条都不满足，即使修复本身是正确的，我们也会关闭这个拉取请求。
