...
새로운 함수가 추가될 때는 벤치마크 코드도 함께 추가해 주세요. Pull Request를 열 때 무엇을 첨부해야 하는지는 [4.1 어떤 기여를 받나요](#41-어떤-기여를-받나요)를 참고해 주세요.
...
Lodash의 타입이 허용하는 입력에 대한 수정은 환영해요. `es-toolkit/compat`은 Lodash와 똑같이 동작하는 게 중요해요. 다른 값을 돌려주면 마이그레이션하는 프로젝트가 원인을 찾기 어려운 방식으로 깨지거든요.

다만 Lodash에는 문자열 자리에 `null`이나 숫자를 넘기는 것처럼 잘못된 입력을 처리하는 오래된 동작들이 있어요. 이런 동작 하나하나가 타입이 있는 프로젝트에서는 작성할 수조차 없는 호출을 위해, 아주 자주 실행되는 코드에 분기를 더해요. 그래서 다음 중 하나에 해당할 때만 차이를 고치는 PR을 받아요:

1. **TypeScript `strict` 모드에서 `@types/lodash`로 컴파일되는 호출이에요.** 재현 코드를 `import { fn } from 'lodash'`로 작성하고 `tsc`로 확인해 주세요. 타입이 호출을 거부하면 그 차이는 범위 밖이에요.
2. **실제 코드가 그렇게 호출하고 있어요.** 그런 입력을 넘기는 GitHub 공개 저장소 파일의 permalink를 링크해 주세요. 다른 사람들이 쓰는 프로젝트여야 해요. 버그를 보여 주려고 만든 저장소, 포크, 그리고 Lodash 자체 테스트 코드는 안 돼요. Lodash 테스트는 Lodash가 어떻게 동작하는지 보여 줄 뿐, 누가 어떻게 쓰는지 보여 주지는 않으니까요.

둘 다 해당하지 않으면 수정이 맞더라도 PR을 닫아요.
