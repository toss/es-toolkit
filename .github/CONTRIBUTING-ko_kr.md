...
## 5. Compat 함수 문서 작성 가이드

`es-toolkit/compat` 함수의 레퍼런스 문서는 정해진 템플릿을 따라요. 모든 compat 문서는 두 가지 목표를 가져요. (1) 더 빠르거나 현대적인 대체 API로 사용자를 자연스럽게 안내해요. (2) 마이그레이션 중에 이 함수를 계속 사용하는 사용자를 위해 동작을 정확히 설명해요.

### 5.1 템플릿

`docs/**/reference/compat/{category}/{fn}.md` 파일은 아래 골격을 사용해요.
